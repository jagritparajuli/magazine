	...
	class database{
		...
		protected function addData($data,$is_die = false){
			try{
				...
				if (isset($this->table)&& !empty($this->table)) {
					...
					throw new Exception("Data cannot be inserted without table name", 1);
				...
				if(isset($data) && !empty($data)){
					...
					throw new Exception("Data cannot be inserted without data", 1);
		...
		protected function getData($args,$is_die=false){
			try{
				...
				//fields
				if(isset($args['fields'])&& !empty($ags['fields'])){
					if(is_array($args)){
